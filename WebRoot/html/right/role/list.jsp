...
<html>
	...
	<body>
		...
		<form action="/role" method="post">
			<hidden property="o" value="toRoleList" />
				...
				<s:iterator value="roles" var="role">
					<tr>
						...
						<c:if test="${role.roleFlag==1}">
							...
							<a href="${basePath}/html/right/role/listRightByRole.action?role.roleId=${role.roleId}">
							...
							<img onclick="to('role.do?o=toDetailSysRole&roleId=${role.roleId}');"
								title="查看" src="${basePath}/html/images/bt_deal.gif" class="op_button" />
						<c:if test="${role.roleFlag==1}">
							<img
								onclick="del('${role.roleName}','role.do?o=toDeleteSysRole&roleId=${role.roleId }');"
								title="删除" src="${basePath}/html/images/bt_del.gif" class="op_button" />
						</c:if>
						</td>
					</tr>
				</s:iterator>
